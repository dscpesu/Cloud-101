...
# Copy the Gemfile and Gemfile.lock from the host to the container
COPY Gemfile Gemfile.lock ./

# Install Ruby gems which are specified in the Gemfile.lock within the container
RUN bundle install

# Copy the rest of your application's files and code to the /app directory in the container
COPY . .
